...
if (software == "d") {
    ...
    write.table(res, file = out, sep = "\t", quote = F, row.names = F, col.names = T)
    ### edgeR
} else if (software == "e") {
    ...
    res <- as.data.frame(topTags(qlf, n = nrow(readCount), adjust.method = "BH", sort.by = "PValue", p.value = fdrcutoff))
    res$Gene <- rownames(res)
    ...
    outputRst <- na.omit(as.data.frame(cbind(
        row.names(count_norm)[which(fdr < fdrcutoff)],
        pvalue[which(fdr < fdrcutoff)],
        fdr[which(fdr < fdrcutoff)]
    ), stringsAsFactors = F))
    ...
    outputRst$Gene <- rownames(outputRst)
    write.table(outputRst, file = out, sep = "\t", quote = F, row.names = F, col.names = T)
    ### dearseq
} else if (software == "r") {
    library(dearseq)
    if (edgeRNorm == TRUE) {
    ...
    res$Gene <- rownames(res)
    write.table(res[res$adjPval < fdrcutoff, ], file = out, sep = "\t", quote = F, row.names = F, col.names = T)
} else {
